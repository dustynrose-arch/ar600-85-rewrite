...
| Guide | Header **User Guide** opens a floating window over the draft (drag to move, resize from the corner; Close or Escape). The draft stays usable. The Guide is text + First-session walkthrough only. `/guide` only returns you to that same window. Assist help is written for G–1 editors. |
