...
**Not an official publication.** Word exports (header, footer, and title page) are always marked **DRAFT / WORKING COPY**. The header has no DRAFT chip or banner. G–1 seal on the left (`public/g1-seal.png`); U.S. Army lockup on the right (`public/us-army-logo.png`).
...
| Area | Behavior |
| --- | --- |
| Chrome | Outline \| editor \| Assistant/authority. Official G–1 seal at `public/g1-seal.png` (left); official U.S. Army lockup at `public/us-army-logo.png` (right). Title *AR 600-85 Revision*. Subtitle *Directorate of Prevention, Resilience and Readiness*. Outline and Assistant collapse so the center editor can widen; the tab remembers that choice for the session. |
| Editing | Section edit with automatic save plus an Editor-only **Save** button (timeline: “Manual save”). The original regulation pane is read-only. Your draft has Undo (button and Ctrl+Z / ⌘Z) and browser spellcheck; the original regulation pane is not spellchecked. Hide outline / Hide Assistant live on those side panes only — not in the center chrome. |
