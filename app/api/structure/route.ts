...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.action === "add") {
      ...
      await addWorkingSection({ targetId: body.targetId, position: body.position, title: body.title }, role, mode);
    } else if (body.action === "delete") {
      if (!body.nodeId) throw new Error("Delete requires nodeId.");
      await deleteWorkingSection(body.nodeId, role, mode);
    } else if (body.action === "move") {
      if (!body.nodeId || !body.parentId || body.index == null) {
        throw new Error("Move requires nodeId, parentId, and index.");
      }
      await moveWorkingSection({ sectionId: body.nodeId, parentId: body.parentId, index: body.index }, role, mode);
    } else if (body.action === "rename") {
      if (!body.nodeId || body.title == null) throw new Error("Rename requires nodeId and title.");
      await renameWorkingSection(body.nodeId, body.title, role, mode);
    } else if (body.action === "split") {
      if (!body.nodeId) throw new Error("Split requires nodeId.");
      await splitWorkingSection(body.nodeId, role, mode);
    } else {
