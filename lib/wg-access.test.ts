...
test("access password field is an uncontrolled native form POST", () => {
  ...
  assert.equal(form.includes("inert"), false);
  assert.equal(form.includes("aria-hidden"), false);
  assert.equal(form.includes("fetch("), false);
  assert.match(form, /htmlFor="wg-access-secret"/);
  assert.match(form, /id="wg-access-secret"/);
  const labelClose = form.indexOf("</label>");
  const passwordAt = form.indexOf('type="password"');
  assert.ok(labelClose > 0 && passwordAt > labelClose, "password input is not nested in the label");
  const css = readFileSync(new URL("../app/globals.css", import.meta.url), "utf8");
  assert.match(css, /\.wg-access-secret \{[\s\S]*caret-color:\s*#f4efe3/);
  assert.match(css, /\.wg-access-secret:focus \{[\s\S]*outline:\s*2px solid #e2b84a/);
  assert.match(css, /\.wg-access-form \{[\s\S]*pointer-events:\s*auto/);
  assert.equal(/\.wg-access-secret \{[\s\S]*pointer-events:\s*none/.test(css), false);
